...
void L1KaonTrackSelectionProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  for (size_t i = 0; i < nOutputApproximate; i++) {
    ...
    if (processSimulatedTracks_ && !TTTrackChargeSelector(track)) {
      vTTPosTrackOutput->push_back(trackRef);
    }
    else {
    ...
    if (processEmulatedTracks_ && !TTTrackWordChargeSelector(track)) {
      vTTPosTrackEmulationOutput->push_back(trackRef);
    }
    else {
