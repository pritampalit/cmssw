...
namespace deep_tau {
  ...
  public:
    ...
    const cms::Ort::ONNXRuntime& getSession(const std::string& name = "") const {
...
class DeepTauIdONNXWrapper
  : public edm::stream::EDProducer<edm::GlobalCache<deep_tau::DeepTauCacheONNX>> {
public:
...
public:
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    desc.add<std::vector<std::string>>("graph_file",
				       //{});
				       {"/afs/cern.ch/work/p/ppalit2/public/test_sonic/CMSSW_16_0_0_pre4/src/onnx_models_new/RecoTauTag-TrainingFiles/DeepTauId/deepTau_2017v2p6_e6_core.onnx"});
    //desc.add<bool>("mem_mapped", false);
    descriptions.add("DeepTauONNX", desc);
  }
  
  explicit DeepTauIdONNX(const edm::ParameterSet& cfg,
			 const deep_tau::DeepTauCacheONNX* cache)
    : DeepTauIdBaseONNX<DeepTauIdONNXWrapper>(cfg), cache_(cache) {
    
    if (version_ != 2)
      throw cms::Exception("DeepTauIdONNX")
	<< "version " << version_ << " is not supported.";
    ...
    if (sub_version_ == 1) {
      ...
      tauBlockSize_ = static_cast<int>(TauBlockInputs::NumberOfInputs) -
	static_cast<int>(TauBlockInputs::varsToDrop.size());
    } else {
      throw cms::Exception("DeepTauIdONNX")
	<< "sub_version " << sub_version_ << " is not supported.";
    }
    ...
    for (size_t n = 0; n < 2; ++n) {
      ...
      hadronBlock_[is_inner].assign(HadronBlockInputs::NumberOfInputs, 0.f);
      zeroConvFeatures_[is_inner] = getPartialPredictions(is_inner, 1);
      //zeroConvFeatures_[is_inner].assign(number_of_conv_features, 0.f);

      // restore to full size for event processing
      eGammaBlock_[is_inner].assign(max_cells * EgammaBlockInputs::NumberOfInputs, 0.f);
      muonBlock_[is_inner].assign(max_cells * MuonBlockInputs::NumberOfInputs, 0.f);
      hadronBlock_[is_inner].assign(max_cells * HadronBlockInputs::NumberOfInputs, 0.f);
      */
  ...
  static std::unique_ptr<deep_tau::DeepTauCacheONNX>
  ...
  void produce(edm::Event& event, const edm::EventSetup& es) override {
    ...
    //std::cout << "!!!!!!!!!!!!!!!!!!!!!!!Here is producer!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
...
private:
  ...
  void setCellConvFeatures(bool is_inner, int eta_index, int phi_index,
  ...
  std::vector<float> getPartialPredictions(bool is_inner, size_t n_valid_cells) {
    ...
    input_values.push_back(std::vector<float>(
        muonBlock_[is_inner].begin(),
        muonBlock_[is_inner].begin() + n_valid_cells * MuonBlockInputs::NumberOfInputs));
    input_shapes.push_back({batch, 1, 1, MuonBlockInputs::NumberOfInputs});

    input_values.push_back(std::vector<float>(
        hadronBlock_[is_inner].begin(),
    ...
    const std::vector<std::string> output_names = {
        is_inner ? "inner_all_dropout_4/Identity" : "outer_all_dropout_4/Identity"};

    auto outputs = cache_->getSession(sess_name).run(
        input_names, input_values, input_shapes, output_names);

    // debug diff
    /*
    float sum = 0.f;
    for (auto v : outputs.at(0)) sum += v;
    std::cout << "ONNX PARTIAL PRED is_inner=" << is_inner << " n_valid_cells=" << n_valid_cells << " sum=" << sum << std::endl;
    */

    //debug diff v2
    /*
    std::cout << "PARTIAL size=" << outputs.at(0).size() 
              << " n_valid_cells=" << n_valid_cells 
              << " expected=" << n_valid_cells * dnn_inputs_v2::number_of_conv_features 
              << std::endl;
    //
    */
    ...
    if (n_valid > 0) {
      ...
      for (int eta = -grid.maxEtaIndex(); eta <= grid.maxEtaIndex(); ++eta) {
        for (int phi = -grid.maxPhiIndex(); phi <= grid.maxPhiIndex(); ++phi) {
          ...
          if (grid.find(cell_index) == grid.end()) continue;

	  //debug diff v3
	  //std::cout << "FILL idx=" << idx << " eta=" << eta << " phi=" << phi << std::endl;
          ...
          createEgammaBlockInputs<CandidateCastType>(
              idx, tau, tau_index, tau_ref, pv, rho,
              electrons, pfCands, cell, tau_funcs, is_inner, eg_it);
          createMuonBlockInputs<CandidateCastType>(
              idx, tau, tau_index, tau_ref, pv, rho,
              muons, pfCands, cell, tau_funcs, is_inner, mu_it);
          createHadronsBlockInputs<CandidateCastType>(
              idx, tau, tau_index, tau_ref, pv, rho,
              pfCands, cell, tau_funcs, is_inner, had_it);
          ++idx;
        }
      }
      ...
      //debug diff v4
      /*
      float sum_partial = 0.f;
      for (auto v : partial) sum_partial += v;
      std::cout << "PARTIAL SUM inside createConvFeatures is_inner=" << is_inner 
		<< " sum=" << sum_partial << std::endl;
      */
      ...
      for (int eta = -grid.maxEtaIndex(); eta <= grid.maxEtaIndex(); ++eta) {
	    ...
	    // debug diff v3
	    //std::cout << "SCATTER idx2=" << idx2 << " eta=" << eta << " phi=" << phi 
            //              << " eta_index=" << eta_idx << " phi_index=" << phi_idx << std::endl;
	    
            setCellConvFeatures(is_inner, eta_idx, phi_idx,
                                &partial[idx2 * number_of_conv_features]);
            ++idx2;
          } else {
            setCellConvFeatures(is_inner, eta_idx, phi_idx,
                                zeroConvFeatures_[is_inner].data());
          }
        }
      }
      ...
      /*
      float sum_conv = 0.f;
      for (auto v : convFeatures_[is_inner]) sum_conv += v;
      std::cout << "CONV SUM after scatter is_inner=" << is_inner 
		<< " sum=" << sum_conv << std::endl;
      */
  ...
  std::vector<float> getPredictionsV2(TauCollection::const_reference& tau,
    ...
    createTauBlockInputs<CandidateCastType>(
					    ...
					    tau_index, tau_ref, pv, rho, tau_funcs, tau_it);
    
    createConvFeatures<CandidateCastType>(
					  dynamic_cast<const TauCastType&>(tau),
					  tau_index, tau_ref, pv, rho,
					  electrons, muons, pfCands, inner_grid, tau_funcs, true);
    createConvFeatures<CandidateCastType>(
					  dynamic_cast<const TauCastType&>(tau),
					  tau_index, tau_ref, pv, rho,
					  electrons, muons, pfCands, outer_grid, tau_funcs, false);
    ...
    // debug for diff
    /*
    float sum_inner = 0.f, sum_outer = 0.f;
    for (auto v : convFeatures_[true])  sum_inner += v;
    for (auto v : convFeatures_[false]) sum_outer += v;
    std::cout << "CONV FEATURES sum_inner=" << sum_inner << " sum_outer=" << sum_outer << std::endl;
    //
    */
    
    auto outputs = cache_->getSession("core").run(
						  input_names, input_values, input_shapes, output_names);
    
    std::vector<float> pred = outputs.at(0);

    //std::cout << "*****************ONNX RAW PRED tau_index=" << tau_index << " e=" << pred[0] << " mu=" << pred[1] << " tau=" << pred[2] << " jet=" << pred[3] << std::endl;
    ...
    /*
    if (save_inputs_) {
      std::string json_file_name = "DeepTauIdONNX_" + std::to_string(eventnr) + "_" + std::to_string(tau_index) + ".json";
    ...
    if (save_inputs_) {
      ...
      auto dumpGrid = [&](const std::string& block_name,
      ...
      dumpGrid("input_inner_egamma", eGammaBlock_[true],
	       dnn_inputs_v2::EgammaBlockInputs::NumberOfInputs, inner_grid);
      dumpGrid("input_inner_muon", muonBlock_[true],
	       dnn_inputs_v2::MuonBlockInputs::NumberOfInputs, inner_grid);
      dumpGrid("input_inner_hadrons", hadronBlock_[true],
	       dnn_inputs_v2::HadronBlockInputs::NumberOfInputs, inner_grid);
      dumpGrid("input_outer_egamma", eGammaBlock_[false],
	       dnn_inputs_v2::EgammaBlockInputs::NumberOfInputs, outer_grid);
      dumpGrid("input_outer_muon", muonBlock_[false],
	       dnn_inputs_v2::MuonBlockInputs::NumberOfInputs, outer_grid);
      dumpGrid("input_outer_hadrons", hadronBlock_[false],
	       dnn_inputs_v2::HadronBlockInputs::NumberOfInputs, outer_grid);
  ...
  std::vector<std::vector<float>> getPredictions(edm::Event& event,
                                                  edm::Handle<TauCollection> taus) {
    ...
    const edm::AssociationVector<reco::PFTauRefProd,
    ...
    if (!is_online_) {
      ...
      if (!discrIndicesMapped_) {
        basicDiscrIndexMap_ = matchDiscriminatorIndices(
            event, basicTauDiscriminators_inputToken_, requiredBasicDiscriminators_);
        basicDiscrdR03IndexMap_ = matchDiscriminatorIndices(
            event, basicTauDiscriminatorsdR03_inputToken_, requiredBasicDiscriminatorsdR03_);
    ...
    std::vector<std::vector<float>> predictions(
        taus->size(), std::vector<float>(deep_tau::NumberOfOutputs, 0.f));
    ...
    for (size_t tau_index = 0; tau_index < taus->size(); ++tau_index) {
      ...
      if (is_online_) {
        passesPrediscriminants =
            tauIDs.passPrediscriminants<std::vector<TauDiscInfo<reco::PFTauDiscriminator>>>(
                recoPrediscriminants_, andPrediscriminants_, tauRef);
      } else {
        passesPrediscriminants =
            tauIDs.passPrediscriminants<std::vector<TauDiscInfo<pat::PATTauDiscriminator>>>(
      ...
      if (passesPrediscriminants) {
        ...
        if (is_online_) {
          pred = getPredictionsV2<reco::PFCandidate, reco::PFTau>(
              taus->at(tau_index), tau_index, tauRef,
              electron_collection, muon_collection,
              *pfCands, vertices->at(0), *rho, eventnr, tauIDs);
        } else {
          pred = getPredictionsV2<pat::PackedCandidate, pat::Tau>(
              taus->at(tau_index), tau_index, tauRef,
              electron_collection, muon_collection,
              *pfCands, vertices->at(0), *rho, eventnr, tauIDs);
        }
